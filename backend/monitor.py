...
# In-memory tracking of alert states (api_id -> boolean)
alert_sent = {}
...
def check_api(api: dict) -> dict:
    ...
    # SSL verification is disabled to support corporate/proxy networks that
    # intercept HTTPS traffic with their own certificates.
    # In a production environment on a trusted network, set verify=True.
    SSL_VERIFY = False
    ...
    try:
        if api_type == "statuspage":
            ...
            try:
                response         = requests.get(json_url, timeout=TIMEOUT_SECONDS, headers=headers, verify=SSL_VERIFY)
                ...
                start            = time.monotonic()
                response         = requests.get(api_url, timeout=TIMEOUT_SECONDS, headers=headers, verify=SSL_VERIFY)
                elapsed          = time.monotonic() - start
                response_time    = round(elapsed * 1000, 2)
                http_status_code = response.status_code
                if response.ok:
    ...
    from backend.database import get_users_watching_api
    from backend.email_alerts import send_alert, send_recovery_alert

    global alert_sent
    if status == "DOWN":
        if not alert_sent.get(api_id, False):
            watching_users = get_users_watching_api(api_id)
            for user in watching_users:
                send_alert(user["email"], api["api_name"], api_url)
            alert_sent[api_id] = True
    elif status == "UP":
        if alert_sent.get(api_id, False):
            watching_users = get_users_watching_api(api_id)
            for user in watching_users:
                send_recovery_alert(user["email"], api["api_name"])
            alert_sent[api_id] = False

    # ── Phase 2: automated failover evaluation ────────────────────────────────
    from backend.failover import evaluate_failover
    evaluate_failover(api_id, status)
...
def calculate_reliability(api_id: int) -> dict:
    ...
    # ── No data guard ─────────────────────────────────────────────────────────
    # When total_checks == 0, avg_rt is 0.0 which would incorrectly produce
    # speed_score=100 and reliability_score=20.0.  Return explicit zeros instead.
    if total == 0:
        return {
            "status":            "no_data",
            "total_checks":      0,
